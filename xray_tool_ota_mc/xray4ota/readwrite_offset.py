...
with open("template_tb/offset_tb.spice", "r") as file:
    offset_tb=file.read().splitlines()
...
Summary = open("spice/offset.spice","w")
for i in range(len(offset_tb)):
    if(i==32):
        string = offset_tb[i]
        L1=string.replace("write "+string,directory+"/dc/offset/rawfiles/offset.raw" )
        Summary.write(L1)
        Summary.write("\n")

    else:
        st= "x is greater than y"
        Summary.write(offset_tb[i])
        Summary.write("\n")
