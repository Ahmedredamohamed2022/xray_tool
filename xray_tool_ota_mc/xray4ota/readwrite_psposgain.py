...
with open("template_tb/psposgain_tb.spice", "r") as file:
    psposgain_tb=file.read().splitlines()
...
Summary = open("spice/psposgain.spice","w")
for i in range(len(psposgain_tb)):
    if(i==25):
        string = psposgain_tb[i]
        L1=string.replace("write "+string,directory+"/ac/pspos/rawfiles/psposgain.raw" )
        Summary.write(L1)
        Summary.write("\n")

    else:
        st= "x is greater than y"
        Summary.write(psposgain_tb[i])
        Summary.write("\n")
