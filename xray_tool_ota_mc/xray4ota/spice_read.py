        ...
        for k,v in kwargs.items():
            if hasattr(self,k):
                if type(getattr(self,k)) == type(v):
                    ...
                    print( "Warning: attribute " + k + " value has wrong type: " \
                          + str(type(v)) + " ignored!" )
            else:
...
class spice_plot(object):
    ...
    def __init__(self, scale=None ,data=None, **kwargs):
        ...
        """
        self.title = b"title undefined"
        self.date = b"date undefined"
        self.plotname = b"plotname undefined"
        self.plottype = b"plottype undefined"
        self.dimensions = []
            ...
            if keyword == b"title":
                ...
                for flag in ftok:
                    if flag == b"real":
                        ...
                        print( 'Warning: unknown flag: "' + str(flag,"utf-8") + '"')
    ...
    for f in sys.argv[1:]:
        ...
        for i,p in enumerate(spice_read(f).get_plots()):
            ...
            for j,d in enumerate(p.get_datavectors()):
                ...
                print( '      Vector-Type: ', v.dtype)
'''      
plotlist = spice_read("/ciic/designs/analog-mixed-signal-blocks/simulations/Level-shifter-mc/mc100.raw").get_plots()

#x = plot2.get_scalevector().get_data()

print(" rddadadsdadadadadasd")
f=spice_read(f).get_plots()[0]
x=f.get_scalevector().get_data()
print(x)
print(len(x))
va = f.get_datavector(0).get_data()
print(va)
vb = f.get_datavector(1).get_data()
print(vb)
print("please plot me")
figure(1)
subplot(311)
plot(x, va, c="red",lw=2,  label="va")
plt.subplot(312)
plt.plot(x, vb, c="red",lw=2,  label="va")
'''
