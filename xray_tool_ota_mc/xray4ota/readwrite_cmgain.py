...
with open("template_tb/cmgain_tb.spice", "r") as file:
    cmgain_tb=file.read().splitlines()
...
Summary = open("spice/cmgain.spice","w")
for i in range(len(cmgain_tb)):
    if(i==34):
        string = cmgain_tb[i]
        L1=string.replace("write "+string,directory+"/ac/cm/rawfiles/cmgain.raw" )
        Summary.write(L1)
        Summary.write("\n")

    else:
        st= "x is greater than y"
        Summary.write(cmgain_tb[i])
        Summary.write("\n")
